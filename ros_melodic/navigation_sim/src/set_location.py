...
class SetLocationServer():
    ...
    def saveLocation(self, file_name):
        try:
            param_path = roslib.packages.get_pkg_dir("navigation_sim")
            map_path = roslib.packages.get_pkg_dir("megarover_samples")
            rospy.set_param('/navigation_sim/location_dict', self.location_dict)
            rosparam.dump_params(param_path + '/location/' + file_name + '.yaml', '/navigation_sim/location_dict')
            print rosparam.get_param('/navigation_sim/location_dict')
